      ...
      <div className="relative h-80 overflow-hidden">
        <div className="absolute inset-0">
          <FullScreenVideoAvatar
            ...
            isHovered={isHovered}
            showControls={false}
            onVideoReady={() => console.log(`Video ready for ${specialist.name}`)}
            onError={(error) => {
              console.error(`Video error for ${specialist.name}:`, error);
              setImageError(true);
            }}
          />
        </div>
        
        {/* Fallback gradient overlay when video fails */}
        {imageError && (
        ...
        {/* Video Control Button (now for demo/intro video, not avatar) */}
        <div 
          className="absolute top-4 left-4 cursor-pointer opacity-0 group-hover:opacity-100 transition-all duration-300"
          onClick={handleVideoClick}
        >
          <div className="w-12 h-12 bg-black/50 backdrop-blur-sm rounded-full flex items-center justify-center hover:bg-black/70 transition-all duration-300 hover:scale-110">
            {isPlaying ? (
